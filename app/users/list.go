...
// @Param        filters          query     []string  false  "Condition for users retrieval, ex. `name:eq:John` | `name:like:smith`"
// @Param        sorts            query     []string  false  "Sort for users data, ex. `created_at:asc`"
// @Success      200              {object}  view.SuccessPaginatorResp{data=view.dataListWithOption{lists=[]inout.UserReadView}}
...
func (ctrl *Controller) List(c *gin.Context) {
	sp, ctx := opentracing.StartSpanFromContext(c, "handler.users.List")
	defer sp.Finish()
