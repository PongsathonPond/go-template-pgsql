...
func (suite *PackageTestSuite) TestFind() {
	...
	total, items, err := suite.repo.Find(suite.ctx, nil, nil, typeGuide)
...
func (suite *PackageTestSuite) TestSearchFind() {
	test1 := suite.makeTestStruct("test", "a", "b")
	_, err := suite.repo.Create(suite.ctx, test1)
	suite.NoError(err)
	test2 := suite.makeTestStruct("test2", "a", "b", "c")
	...
	total, items, err := suite.repo.Find(suite.ctx, filters, nil, typeGuide)
